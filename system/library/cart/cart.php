...
class Cart
{
    ...
    {
        $product_data = array();
        foreach ($this->getCartProducts() as $value) {
            if ($value['selected']) {
                $product_data[] = $value;
            }
        }
        ...
        if (!isset($this->data[$this->buy_type]) || !$this->data[$this->buy_type]) {
            ...
            foreach ($cart_query->rows as $cart) {
                ...
                if ($product_query->num_rows && ($cart['quantity'] > 0)) {
                    ...
                    if ($variantData = \Models\Product::find($cart['product_id'])->getVariantLabels()) {
                    ...
                    // flash sale
                    $flash_date = \Flash::getSingleton()->getFlashPriceAndCount($cart['product_id']);
                    if ($flash_date && $flash_date['price'] !== false) {
                        $price = $flash_date['price'];
                    }
    ...
    public function add($product_id, $quantity = 1, $option = array(), $recurring_id = 0)
    {
        ...
        if (!$query->row['total']) {
            $this->db->query("INSERT " . DB_PREFIX . "cart SET api_id = '" . (isset($this->session->data['api_id']) ? (int)$this->session->data['api_id'] : 0) . "', customer_id = '" . (int)$this->customer->getId() . "', session_id = '" . $this->db->escape($this->session->getId()) . "', product_id = '" . (int)$product_id . "', recurring_id = '" . (int)$recurring_id . "', `option` = '" . $this->db->escape(json_encode($option)) . "', quantity = '" . (int)$quantity . "', selected = '1', buy_type = '" . $this->buy_type . "',date_added = NOW()");
        } else {
    ...
    public function clear()
    {
        $customer_id        = (int)$this->customer->getId();
        if ($customer_id > 0) {
            $this->db->query("DELETE FROM " . DB_PREFIX . "cart WHERE customer_id = '" . $customer_id . "' AND selected = '1' AND session_id = '" . $this->db->escape($this->session->getId()) . "' AND buy_type = '" . $this->buy_type . "'");
        }else{

            $this->db->query("DELETE FROM " . DB_PREFIX . "cart WHERE api_id = '" . (isset($this->session->data['api_id']) ? (int)$this->session->data['api_id'] : 0) . "' AND customer_id = '" . $customer_id . "' AND selected = '1' AND session_id = '" . $this->db->escape($this->session->getId()) . "' AND buy_type = '" . $this->buy_type . "'");
        }
    }
    ...
    /*
     * return： true 需要配送地址和配送方式，false 不需要配送地址和配送方式
     */
    public function hasShipping()
    {
        ...
        if ($shipping && isset($this->session->data['is_pickup']) && $this->session->data['is_pickup']) {
            $shipping = false;
        }
    ...
    /*
     * return： true 商品本身需要配送（实体商品），false 商品本身不需要配送（虚拟商品）
     */
    ...
    public function select($selects = array())
    {
        ...
        foreach ($products as $value) {
            if (in_array($value['cart_id'], $selects)) {
                $this->db->query("UPDATE " . DB_PREFIX . "cart SET selected = '1' WHERE cart_id = '" . (int)$value['cart_id'] . "'");
            } else {
    ...
    public function getCartProductCount($product_id)
    {
        $quantity = 0;
