...
if (!defined('DIR_OCROOT')) {
    define('DIR_OCROOT', dirname(DIR_APPLICATION) . '/');
}
// Autoloader
if (is_file(DIR_OCROOT . 'vendor/autoload.php')) {
	require_once(DIR_OCROOT . 'vendor/autoload.php');
} else {
    echo '请在命令行运行 composer install 或联系我们客服人员';
    exit;
}

function library($class) {
    $file = DIR_SYSTEM . 'library/' . str_replace('\\', '/', strtolower($class)) . '.php';
    $modelFile = DIR_SYSTEM . str_replace('\\', '/', strtolower($class)) . '.php';

    if (is_file($file)) {
        include_once(modification($file));
        return true;
    } elseif (is_file($modelFile)) {
        include_once(modification($modelFile));
        return true;
    } else {
        return false;
    }
...
require_once(DIR_SYSTEM . 'engine/admin_controller.php');
