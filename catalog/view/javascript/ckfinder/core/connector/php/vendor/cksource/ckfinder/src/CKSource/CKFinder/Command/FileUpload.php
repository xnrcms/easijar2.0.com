...
class FileUpload extends CommandAbstract
{
    ...
    public function execute(Request $request, WorkingFolder $workingFolder, EventDispatcher $dispatcher, Config $config, CacheManager $cache, ThumbnailRepository $thumbsRepository)
    {
        ...
        $session = $_SESSION;
        $image_base_path =  $session['image_base_path'];
        $water_data =  $session['water_data'];
        if (isset($_SESSION['seller_id']) && $_SESSION['seller_id']) {
            $seller_folder = $_SESSION['seller_id'];
        } else {
            $seller_folder = 'default';
        }
        if($water_data['status']){
            if($water_data['type'] == '1'){
                $this->waterImageAlpha($image_base_path . 'catalog/seller_products/' . $seller_folder . '/' . $currentFile, $image_base_path . '/' . $water_data['img'], $water_data['alpha'], $water_data['pos']);
            }elseif($water_data['type'] == '2'){
                $this->waterFont($image_base_path . 'catalog/seller_products/' . $seller_folder . '/' . $currentFile, $water_data['font'], $water_data['font_src'], $water_data['font_color'], $water_data['pos'], $water_data['alpha']);
            }
        }
        /** watermark end  */
    ...
    protected function waterFont($img, $water_font, $font_src, $font_color, $water_pos = 'lt', $alpha, $font_size = 20){
        ...
        if($image_width >= 300 || $image_height >= 300){
            ...
            $black = imagecolorallocatealpha($dst, (int)$color_r, (int)$color_g, (int)$color_b, $in_alpha);
            ...
            switch($water_pos){
                ...
                case 'lb':
                    ...
                    break;
                case 'rb':
                    $pos_x = $image_width - $waterSize['font_width'] - 10;
                    $pos_y = $image_height - 10;
                    break;
                case 'cc':
