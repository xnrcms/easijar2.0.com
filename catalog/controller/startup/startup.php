...
class ControllerStartupStartup extends Controller {
	public function index() {
		...
		// 设置手机模式及手机主题
		if (is_mobile()) {
			$this->config->set('is_mobile', true);

			if (config('config_theme_mobile') && config('theme_' . config('config_theme_mobile') . '_status')) {
				$this->config->set('config_theme', config('config_theme_mobile'));
			}
		}

		$this->config->set('is_tablet', is_tablet());
		$this->config->set('is_weixin', is_weixin());
		$this->config->set('is_mobile', is_mobile());
		$this->config->set('is_desktop', is_desktop());
		...
		// Recharge
		if (isset($this->request->get['route']) && strpos($this->request->get['route'], 'checkout/checkout') !== 0 && isset($this->session->data['recharges'])) {
