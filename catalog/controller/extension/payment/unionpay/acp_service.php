...
class AcpService {
	...
	static function validate($params, $cert_dir=SDK_VERIFY_CERT_DIR ) {
		$logger = LogUtil::getLogger();
		// 公钥
		$public_key = CertUtil::getVerifyCertByCertId ( $params ['certId'], $cert_dir=SDK_VERIFY_CERT_DIR );
//	echo $public_key.'<br/>';
		// 签名串
...
//	echo date('Y-m-d',time());
	...
	static function validateAppResponse($jsonData, $cert_dir=SDK_VERIFY_CERT_DIR ) {
		...
		$public_key = CertUtil::getVerifyCertByCertId( $dataMap ['cert_id'], $cert_dir=SDK_VERIFY_CERT_DIR  );
