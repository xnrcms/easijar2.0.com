...
class ControllerCommonHeader extends Controller {
    public function index() {
        ...
        if (is_file(DIR_IMAGE . $this->config->get('config_icon'))) {
            $this->document->addLink($this->url->imageLink(config('config_icon')), 'icon');
        }

        // 加载自定义 CSS 文件
        if ($custom_css = $this->load->controller('common/custom_css')) {
            $this->document->addStyle($this->url->imageLink($custom_css));
        }
        ...
        $data['modal'] = array_get($this->request->get, 'modal');
