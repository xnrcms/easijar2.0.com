...
class ControllerProductProduct extends Controller {
	...
	public function index() {
		...
		if (!isset($this->request->get['path'])) {
			$cpath = $this->model_catalog_category->getCategoriePathByProductId($this->request->get['product_id']);
			if (!empty($cpath)) {
				$this->request->get['path'] = $cpath;
			}
		}
		...
		if ($product_info) {
			...
			if (!config('is_mobile')) {
				...
				$this->document->addScript('catalog/view/theme/mobile/javascript/imagesloaded.pkgd.min.js');
			}
			...
			$data['tab_review'] = sprintf($this->language->get('tab_review'), $this->model_account_oreview->getTotalOreviewsByProductId($product_id));
			...
			// flash sale
            $data['flash_sale_status'] = 0;
            $data['buy_limit'] = 0;
            $data['show_flash_sale'] = false;
            $data['flash'] = false;
            $flash_data = Flash::getSingleton()->getFlashPriceAndCount($product_id);
            if ($flash_data && $flash_data['price']) {
                $data['flash_sale_status'] = $this->config->get('module_flash_sale_status');
                ...
                $data['flash'] = $this->currency->format($this->tax->calculate($flash_data['price'], $product_info['tax_class_id'], $this->config->get('config_tax')), $this->session->data['currency']);
                $data['show_flash_sale'] = true;
                $data['buy_limit'] = $flash_data['count'];
                $data['date_start'] = $this->config->get('module_flash_sale_start');
			...
			if ($data['flash_sale_status'] && $data['buy_limit'] && $data['buy_limit'] < $data['minimum']) {
				$data['buy_limit'] = $data['minimum'];
			}

			$data['text_limit'] = sprintf(t('text_limit'), $data['buy_limit']);

            $product_id = $this->request->get['product_id'];
            $data['showInProductPage'] = $this->model_catalog_product->getProductQuestions($product_id);

            $this->load->language('product/askquestion');

			$data['review_status'] = config('config_review_status');

			if (config('config_review_guest') || $this->customer->isLogged()) {
			...
			$data['reviews'] = sprintf($this->language->get('text_reviews'), (int)$this->model_account_oreview->getTotalOreviewsByProductId($product_id));
			...
			$related_width = $this->config->get('theme_' . $this->config->get('config_theme') . '_image_related_width');
			$related_height = $this->config->get('theme_' . $this->config->get('config_theme') . '_image_related_height');

			foreach ($results as $result) {
				$data['products'][] = $this->model_catalog_product->handleSingleProduct($result, $related_width, $related_height);
			}
			...
			// Product specific twig
			if ($product_info['twig']) {
				$twig = trim($product_info['twig']);
