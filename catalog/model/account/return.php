...
class ModelAccountReturn extends Model {
	...
	{
		$fields 	= 'rh.date_added, rs.name AS status, rh.comment';
		$sql 		= "SELECT ".$fields." FROM " . get_tabname('return_history') . " rh LEFT JOIN " . get_tabname('return_status') . " rs ON rh.return_status_id = rs.return_status_id WHERE rh.return_id = '" . (int)$return_id . "' AND rs.language_id = '" . (int)$this->config->get('config_language_id') . "' ORDER BY rh.date_added ASC";
	...
	{
		$query = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "return` WHERE customer_id = '" . $this->customer->getId() . "' AND order_id = '" . (int)$order_id . "' AND product_id = '" . (int)$order_product_id . "' AND return_status_id > 0 AND return_status_id <> 8");
		return $query->row['total'];
	}

	public function getReturnForMs($return_id) {
		$fields 		= format_find_field('return_id,order_id,product_id,fullname,email,telephone,product,model,image,quantity,return_status_id,seller_id,comment,date_ordered,date_added,date_modified,return_reason_id,overtime,is_service,is_receive,order_status_id,responsibility','r');
