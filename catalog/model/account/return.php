...
class ModelAccountReturn extends Model {
	...
	public function getReturnForMs($return_id) {
		$fields 		= format_find_field('return_id,order_id,fullname,email,telephone,product,model,image,quantity,return_status_id,seller_id,comment,date_ordered,date_added,date_modified,return_reason_id','r');
	...
	public function getReturnIdByOrderProductId($order_id = 0,$product_id = 0)
	{
		...
		return (isset($query->row['return_id']) &&  !in_array($query->row['return_status_id'], [3,8])) ? (int)$query->row['return_id'] : 0;
	}
}
