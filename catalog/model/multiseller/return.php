...
//	}
	...
	public function getReturn($return_id) {
		$query = $this->db->query("SELECT DISTINCT *, (SELECT c.fullname FROM " . DB_PREFIX . "customer c WHERE c.customer_id = r.customer_id) AS customer, (SELECT rs.name FROM " . DB_PREFIX . "return_status rs WHERE r.seller_id = '" . $this->customer->getId() . "' AND rs.return_status_id = r.return_status_id AND rs.language_id = '" . (int)$this->config->get('config_language_id') . "') AS return_status FROM `" . DB_PREFIX . "return` r WHERE r.return_id = '" . (int)$return_id . "'");
	...
	public function getReturns($data = array()) {
		...
		if ($implode) {
			$sql .= " WHERE r.seller_id = '" . $this->customer->getId() . "' AND " . implode(" AND ", $implode);
		} else {
			$sql .= " WHERE r.seller_id = '" . $this->customer->getId() . "' ";
        }
	...
	public function getTotalReturnsByReturnStatusId($return_status_id) {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "return` WHERE seller_id = '" . $this->customer->getId() . "' AND return_status_id = '" . (int)$return_status_id . "'");

		return $query->row['total'];
	}

	public function getTotalReturnsByReturnReasonId($return_reason_id) {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "return` WHERE seller_id = '" . $this->customer->getId() . "' AND return_reason_id = '" . (int)$return_reason_id . "'");

		return $query->row['total'];
	}

	public function getTotalReturnsByReturnActionId($return_action_id) {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "return` WHERE seller_id = '" . $this->customer->getId() . "' AND return_action_id = '" . (int)$return_action_id . "'");

		return $query->row['total'];
	}
	
	public function addReturnHistory($return_id, $return_status_id, $comment, $notify) {
		$this->db->query("UPDATE `" . DB_PREFIX . "return` SET `return_status_id` = '" . (int)$return_status_id . "', date_modified = NOW() WHERE seller_id = '" . $this->customer->getId() . "' AND return_id = '" . (int)$return_id . "'");
	...
	public function addReturnHistoryForMs($return_id, $return_status_id, $proposal = 0,$return_reason_id = 0,$comment = '',$evidences = '',$utype = 0) {
		...
		$this->db->query("INSERT INTO `" . DB_PREFIX . "return_history` SET `return_id` = '" . (int)$return_id . "', return_status_id = '" . (int)$return_status_id . "', proposal = '" . (int)$proposal . "', comment = '" . $this->db->escape(strip_tags($comment)) . "',customer_id = '" . $this->customer->getId() . "',evidences = '" . $evidences . "',utype = '" . (int)$utype . "',return_reason_id = '" . (int)$return_reason_id . "', date_added = NOW()");
	}
