...
class ModelCatalogCategory extends Model {
	public function addCategory($data) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "category SET parent_id = '" . (int)$data['parent_id'] . "', `top` = '" . (isset($data['top']) ? (int)$data['top'] : 0) . "', `column` = '" . (int)$data['column'] . "', sort_order = '" . (int)$data['sort_order'] . "', status = '" . (int)$data['status'] . "', twig = '" . str_replace('.twig', '', $this->db->escape((string)$data['twig'])) . "', date_modified = NOW(), date_added = NOW()");
		...
		if (isset($data['category_seo_url'])) {
			...
			foreach ($data['category_seo_url'] as $store_id => $language) {
				foreach ($language as $language_id => $keyword) {
					if (empty($keyword) && (boolean)config('seo_url_setting_category_status')) {
						$name = array_get($data, "category_description.{$language_id}.name");
						$keyword = $this->model_design_seo_url->createUniqueKeyword($name, $language_id, $store_id, 'category', $category_id);
					}
	...
	public function editCategory($category_id, $data) {
		$this->db->query("UPDATE " . DB_PREFIX . "category SET parent_id = '" . (int)$data['parent_id'] . "', `top` = '" . (isset($data['top']) ? (int)$data['top'] : 0) . "', `column` = '" . (int)$data['column'] . "', sort_order = '" . (int)$data['sort_order'] . "', status = '" . (int)$data['status'] . "', twig = '" . str_replace('.twig', '', $this->db->escape((string)$data['twig'])) . "', date_modified = NOW() WHERE category_id = '" . (int)$category_id . "'");
