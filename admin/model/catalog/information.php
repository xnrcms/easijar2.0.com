...
class ModelCatalogInformation extends Model {
	public function addInformation($data) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "information SET sort_order = '" . (int)$data['sort_order'] . "', bottom = '" . (isset($data['bottom']) ? (int)$data['bottom'] : 0) . "', status = '" . (int)$data['status'] . "', twig = '" . str_replace('.twig', '', $this->db->escape($data['twig'])) . "'");
		...
		if (isset($data['information_seo_url'])) {
			...
			foreach ($data['information_seo_url'] as $store_id => $language) {
				foreach ($language as $language_id => $keyword) {
					if (empty($keyword) && (boolean)config('seo_url_setting_information_status')) {
						$title = array_get($data, "information_description.{$language_id}.title");
						$keyword = $this->model_design_seo_url->createUniqueKeyword($title, $language_id, $store_id, 'information', $information_id);
					}
	...
	public function editInformation($information_id, $data) {
		$this->db->query("UPDATE " . DB_PREFIX . "information SET sort_order = '" . (int)$data['sort_order'] . "', bottom = '" . (isset($data['bottom']) ? (int)$data['bottom'] : 0) . "', status = '" . (int)$data['status'] . "', twig = '" . str_replace('.twig', '', $this->db->escape($data['twig'])) . "' WHERE information_id = '" . (int)$information_id . "'");
