...
class ModelSaleReturn extends Model {
	...
	public function getReturn($return_id) {
		$query = $this->db->query("SELECT DISTINCT *, (SELECT c.fullname FROM " . DB_PREFIX . "customer c WHERE c.customer_id = r.customer_id) AS customer, (SELECT rs.name FROM " . DB_PREFIX . "return_status rs WHERE rs.return_status_id = r.return_status_id AND rs.language_id = '" . (int)$this->config->get('config_language_id') . "') AS return_status FROM `" . DB_PREFIX . "return` r WHERE r.return_id = '" . (int)$return_id . "'");
	...
	public function addReturnHistory($data) {
		...
		$overtime 			= isset($data['overtime']) ? (int)$data['overtime'] : 0;

		$this->db->query("UPDATE `" . DB_PREFIX . "return` SET `return_status_id` = '" . (int)$data['return_status_id'] . "',`responsibility` = '" . $responsibility . "',overtime = '" . $overtime . "',`date_modified` = NOW() WHERE return_id = '" . (int)$data['return_id'] . "'");
	...
	public function getTotalReturnHistoriesUtype($return_id,$utype = 0) {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "return_history` WHERE return_id = '" . (int)$return_id . "' AND utype = '" . (int)$utype . "'");
