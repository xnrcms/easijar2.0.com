...
class ModelLocalisationLanguage extends Model {
	public function addLanguage($data) {
		...
		// Variant
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "variant_description WHERE language_id = '" . (int)$this->config->get('config_language_id') . "'");
		foreach ($query->rows as $variant) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "variant_description SET variant_id = '" . (int)$variant['variant_id'] . "', language_id = '" . (int)$language_id . "', name = '" . $this->db->escape($variant['name']) . "'");
		}

		// Variant Value
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "variant_value_description WHERE language_id = '" . (int)$this->config->get('config_language_id') . "'");
		foreach ($query->rows as $variant_value) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "variant_value_description SET variant_value_id = '" . (int)$variant_value['variant_value_id'] . "', language_id = '" . (int)$language_id . "', variant_id = '" . (int)$variant_value['variant_id'] . "', name = '" . $this->db->escape($variant_value['name']) . "'");
		}
		...
		foreach ($query->rows as $product) {
			...
			$this->cache->delete('product.id' . (int)$product['product_id']);
		}
    ...
    {
        $frontendLanguageDir = DIR_APPLICATION . 'language/';
        $folders = glob($frontendLanguageDir . '*', GLOB_ONLYDIR);
        $languages = array();
        foreach ($folders as $path) {
            $languageCode = basename($path);
            $language = $this->getLanguageData($languageCode, $path);
            if (!$language) {
                continue;
            }
            $languages[] = $language;
    ...
    {
        $frontendLanguageDir = DIR_CATALOG . 'language/';
        $folders = glob($frontendLanguageDir . '*', GLOB_ONLYDIR);
        $languages = array();
        foreach ($folders as $path) {
            $languageCode = basename($path);
    ...
    public function getLanguageData($languageCode, $path = null)
    {
        if (!$path) {
            $path = DIR_CATALOG . 'language/' . $languageCode;
        }
        $languageConfig = "{$path}/{$languageCode}.php";
        if (!is_file($languageConfig)) {
            return false;
        } else {
            require $languageConfig;
            $config = $_;
        }
        $dbLanguage = $this->getLanguageByCode($languageCode);
        $language['code'] = $languageCode;
        $language['lang_name'] = $config['lang_name'];
        $language['lang_locale'] = $config['lang_locale'];
        $language['name'] = $config['lang_name'];
        $language['locale'] = $config['lang_locale'];
        $language['language_id'] = 0;
        $language['sort_order'] = 0;
        $language['installed'] = 0;
        $language['status'] = 1;
        $language['name'] = $config['lang_name'];
        if ($dbLanguage) {
            $language['name'] = $dbLanguage['name'];
            $language['language_id'] = $dbLanguage['language_id'];
            $language['sort_order'] = $dbLanguage['sort_order'];
            $language['installed'] = 1;
            $language['status'] = $dbLanguage['status'];
        }
        return $language;
    }
