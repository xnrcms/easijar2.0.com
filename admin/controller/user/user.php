...
class ControllerUserUser extends Controller {
	...
	protected function getForm() {
        ...
        $data['languages'] = $this->model_localisation_language->getAdminLanguages();

        if (isset($this->request->post['default_language'])) {
            $data['default_language'] = $this->request->post['default_language'];
        } elseif (!empty($user_info)) {
            $data['default_language'] = $user_info['default_language'];
        } else {
        ...
        if (isset($this->request->get['filter_name']) || isset($this->request->get['filter_customer']) || isset($this->request->get['filter_email'])) {
            ...
            foreach ($results as $result) {
                ...
                $json[] = array(
                    ...
                    'username' => strip_tags(html_entity_decode($name, ENT_QUOTES, 'UTF-8')),
