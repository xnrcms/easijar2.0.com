...
class ControllerCustomerCustomer extends Controller {
	...
	protected function getList() {
		...
		foreach ($results as $result) {
			...
			$data['customers'][] = array(
				...
				'from'           => $this->getRegisterTypeTextByCode($result['from']),
		...
		$data['sort_telephone'] = $this->url->link('customer/customer', 'user_token=' . $this->session->data['user_token'] . '&sort=c.telephone' . $url);
		...
		$data['sort_from'] = $this->url->link('customer/customer', 'user_token=' . $this->session->data['user_token'] . '&sort=c.from' . $url);
	...
	private function getRegisterTypeTextByCode($code = '')
	{
		if (empty($code)) {
			return;
		}
		...
		$text = t("text_{$code}");
		return $text == "text_{$code}" ? $code : $text;
	}
	...
	protected function validateForm() {
		...
		if (!isset($this->request->get['customer_id'])) { // 创建新客户
		...
		if (!empty($this->request->post['email'])) {
            $customer_info_email = $this->model_customer_customer->getCustomerByEmail($this->request->post['email']);

            if ($customer_info_email && $customer_info_email['customer_id'] != $this->request->get['customer_id']) {
                $this->error['warning'] = $this->language->get('error_exists');
            }
        }

		if (!empty($this->request->post['telephone'])) {
            $customer_info_telephone = $this->model_customer_customer->getCustomerByEmail($this->request->post['telephone']);

			if ($customer_info_telephone && $customer_info_telephone['customer_id'] != $this->request->get['customer_id']) {
    ...
    public function authentication()
    {
        ...
        $customer = Models\Customer::findOrFail($customer_id);
        $data['authentications'] = array();
        $results = $customer->authentications()->offset($page - 1)->limit(10)->get();

        foreach ($results as $key => $result) {
            $data['authentications'][$key] = $result;
            $data['authentications'][$key]['date_added'] = date($this->language->get('datetime_format'), strtotime($result['date_added']));
        }
        $authenticationTotal = $customer->authentications->count();

        $pagination = new Pagination();
        $pagination->total = $authenticationTotal;
        $pagination->page = $page;
        $pagination->limit = 10;
        $pagination->url = $this->url->link('customer/customer/authentication', 'user_token=' . $this->session->data['user_token'] . '&customer_id=' . $customer_id . '&page={page}');

        $data['pagination'] = $pagination->render();

        $data['results'] = sprintf($this->language->get('text_pagination'), ($authenticationTotal) ? (($page - 1) * 10) + 1 : 0, ((($page - 1) * 10) > ($authenticationTotal - 10)) ? $authenticationTotal : ((($page - 1) * 10) + 10), $authenticationTotal, ceil($authenticationTotal / 10));

        $this->response->setOutput($this->load->view('customer/customer_authentication', $data));
    }
