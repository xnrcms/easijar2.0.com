...
class ControllerLocalisationLanguage extends Controller {
    ...
    public function install()
    {
        ...
        $languageCode = $this->request->get['language'];
        if ($languageCode) {
            $checkExist = $this->model_localisation_language->getLanguageByCode($languageCode);
            if (empty($checkExist)) {
                $languageData = $this->model_localisation_language->getLanguageData($languageCode);
	...
	protected function getList() {
		...
		$results = $this->model_localisation_language->getFrontendLanguages($filter_data);

		foreach ($results as $result) {
			$data['languages'][] = array(
				'installed'    => $result['installed'],
				'install_url'  => $this->url->link('localisation/language/install', 'user_token=' . $this->session->data['user_token'] . '&language=' . $result['code'] . $url),
				'language_id' => $result['language_id'],
				'lang_name'   => $result['lang_name'] . (($result['code'] == $this->config->get('config_language')) ? $this->language->get('text_default') : null),
				'code'        => $result['code'],
		...
		$pagination->url = $this->url->link('localisation/language', 'user_token=' . $this->session->data['user_token'] . $url . '&page={page}');
