...
class ControllerSettingSetting extends Controller {
	...
	public function index() {
        ...
        $this->document->addScript('view/javascript/ajaxfileupload.js');
		...
		if (isset($this->error['mail_smtp_username'])) {
			$data['error_mail_smtp_username'] = $this->error['mail_smtp_username'];
		} else {
			$data['error_mail_smtp_username'] = '';
		}
		...
		if (isset($this->error['sendcloud_user'])) {
			$data['error_sendcloud_user'] = $this->error['sendcloud_user'];
		} else {
			$data['error_sendcloud_user'] = '';
		}
		if (isset($this->error['sendcloud_key'])) {
			$data['error_sendcloud_key'] = $this->error['sendcloud_key'];
		} else {
			$data['error_sendcloud_key'] = '';
		}
		...
		// 手机主题
		if (isset($this->request->post['config_theme_mobile'])) {
			$data['config_theme_mobile'] = $this->request->post['config_theme_mobile'];
		} else {
			$data['config_theme_mobile'] = $this->config->get('config_theme_mobile');
		}

		$data['store_url'] = HTTP_CATALOG;

		$data['themes'] = array();
		$data['themes_mobile'] = array(); // 手机主题
		...
		foreach ($extensions as $code) {
			if ($this->config->get('theme_' . $code . '_status')) {
				...
				if (starts_with($code, 'mobile')) { // 以 `mobile` 开头的为手机主题
					$data['themes_mobile'][] = array(
		...
		$data['thumb_placeholder'] = $this->model_tool_image->resize('placeholder.png', 100, 100);
        $data['thumb_no_image'] = $this->model_tool_image->resize('no_image.png', 100, 100);
        $data['user_token'] = $this->session->data['user_token'];
		...
		$data['admin_languages'] = $this->model_localisation_language->getAdminLanguages();
		...
		if (isset($this->request->post['config_review_approve'])) {
			$data['config_review_approve'] = $this->request->post['config_review_approve'];
		} else {
			$data['config_review_approve'] = $this->config->get('config_review_approve');
		}
		...
		if (isset($this->request->post['config_checkout_pickup'])) {
			$data['config_checkout_pickup'] = $this->request->post['config_checkout_pickup'];
		} else {
			$data['config_checkout_pickup'] = $this->config->get('config_checkout_pickup');
		}
        ...
        if (isset($this->request->post['config_unpaid_status_id'])) {
            $data['config_unpaid_status_id'] = $this->request->post['config_unpaid_status_id'];
        } elseif ($this->config->get('config_unpaid_status_id')) {
            $data['config_unpaid_status_id'] = $this->config->get('config_unpaid_status_id');
        } else {
            $data['config_unpaid_status_id'] = 0;
        }

        if (isset($this->request->post['config_paid_status'])) {
            $data['config_paid_status'] = $this->request->post['config_paid_status'];
        } elseif ($this->config->get('config_paid_status')) {
            $data['config_paid_status'] = $this->config->get('config_paid_status');
        } else {
            $data['config_paid_status'] = array();
        }

        if (isset($this->request->post['config_cancelled_status_id'])) {
            $data['config_cancelled_status_id'] = $this->request->post['config_cancelled_status_id'];
        } elseif ($this->config->get('config_cancelled_status_id')) {
            $data['config_cancelled_status_id'] = $this->config->get('config_cancelled_status_id');
        } else {
            $data['config_cancelled_status_id'] = 0;
        }

        if (isset($this->request->post['config_complete_time'])) {
            $data['config_complete_time'] = $this->request->post['config_complete_time'];
        } elseif ($this->config->get('config_complete_time')) {
            $data['config_complete_time'] = $this->config->get('config_complete_time');
        } else {
            $data['config_complete_time'] = 0;
        }

        if (isset($this->request->post['config_unpaid_time'])) {
            $data['config_unpaid_time'] = $this->request->post['config_unpaid_time'];
        } elseif ($this->config->get('config_unpaid_time')) {
            $data['config_unpaid_time'] = $this->config->get('config_unpaid_time');
        } else {
            $data['config_unpaid_time'] = 0;
        }

        if (isset($this->request->post['config_shipped_status_id'])) {
            $data['config_shipped_status_id'] = $this->request->post['config_shipped_status_id'];
        } else {
            $data['config_shipped_status_id'] = $this->config->get('config_shipped_status_id');
        }
		...
		if (isset($this->request->post['config_sendcloud_user'])) {
			$data['config_sendcloud_user'] = $this->request->post['config_sendcloud_user'];
		} elseif ($this->config->has('config_sendcloud_user')) {
			$data['config_sendcloud_user'] = $this->config->get('config_sendcloud_user');
		} else {
			$data['config_sendcloud_user'] = '';
		}

		if (isset($this->request->post['config_sendcloud_key'])) {
			$data['config_sendcloud_key'] = $this->request->post['config_sendcloud_key'];
		} elseif ($this->config->has('config_sendcloud_key')) {
		   	$data['config_sendcloud_key'] = $this->config->get('config_sendcloud_key');
		} else {
			$data['config_sendcloud_key'] = '';
		}
		...
		if (isset($this->request->post['config_license'])) {
			$data['config_license'] = $this->request->post['config_license'];
		} else {
			$data['config_license'] = $this->config->get('config_license');
		}
	...
	protected function validate() {
		...
		if ($this->request->post['config_mail_smtp_username'] && $this->request->post['config_mail_engine'] == 'smtp' && ($this->request->post['config_mail_smtp_username'] != $this->request->post['config_email'])) {
			$this->error['mail_smtp_username'] = $this->language->get('error_mail_smtp_username');
        }
		...
		$config_mail_engine = array_get($this->request->post, 'config_mail_engine');

		if ($config_mail_engine == 'sendcloud') {
			if (!$apiUser = array_get($this->request->post, 'config_sendcloud_user')) {
				$this->error['sendcloud_user'] = $this->language->get('error_sendcloud_user');
			}
			if (!$apiKey = array_get($this->request->post, 'config_sendcloud_key')) {
				$this->error['sendcloud_key'] = $this->language->get('error_sendcloud_key');
			}
			if ($apiUser && $apiKey) {
                $sendCloud = new SendCloudApi($apiUser, $apiKey);
                $sendCloudUser = $sendCloud->getUserInfo();
                if (!$sendCloudUser['result']) {
                    $this->error['sendcloud_key'] = $sendCloudUser['message'];
                }
            }
        }
    ...
    public function uploadImage() {
        ...
        $file_name = $keys[0];
        $image_data = $_FILES[$file_name];
        $json = $this->checkImage($file_name, $image_data);
        $this->response->addHeader('Content-Type: application/json');
        $this->response->setOutput(json_encode($json));
    }
    protected function checkImage($file_name, $data) {
        if (!$this->permission()) {
            return array(
                'code' => 0,
                'message' => $this->language->get('error_upload_image')
            );
        }
        $this->checkDir();
        $this->load->language('setting/setting');
        ...
        if($data['size'] > 2000000) {
            $json = array(
                'code' => 0,
                'message' => $this->language->get('error_size')
            );
        } elseif (!in_array('fileinfo', $opened_extensions)){
            $json = array(
                'code' => 0,
                'message' => $this->language->get('error_extension')
            );
        } else {
            $finfo = new finfo(FILEINFO_MIME_TYPE);
            $file_type = $finfo->file($data['tmp_name']);
            $allow_image_type = array('image/png');
            if(!in_array($file_type, $allow_image_type)) {
                $json = array(
                    'code' => 0,
                    'message' => $this->language->get('error_upload_type')
                );
            } else {
                if($file_name == 'file_placeholder') {
                    $image = 'placeholder/placeholder.png';
                    $image_show = "placeholder/placeholder_" . time() . ".png";
                    $prefix = 'placeholder';
                } else {
                    $image = 'placeholder/no_image.png';
                    $image_show = "placeholder/no_image_" . time() . ".png";
                    $prefix = 'no_image';
                }
                $this->delImage($prefix);
                if(@move_uploaded_file($data['tmp_name'], DIR_IMAGE . $image) && @copy(DIR_IMAGE . $image, DIR_IMAGE . $image_show)) {
                    $this->load->model('tool/image');
                    $json = array(
                        'code' => 1,
                        'thumb' => $this->model_tool_image->resize($image_show, 100, 100)
                    );
                } else {
                    $json = array(
                        'code' => 0,
                        'message' => $this->language->get('error_upload_move')
                    );
                }
            }
        }
    ...
    {
        if(!is_dir(DIR_IMAGE . 'placeholder')) {
            @mkdir(DIR_IMAGE . 'placeholder', '0777', true);
        }
    }

    protected function delImage($prefix)
    {
        $handle = opendir(DIR_IMAGE . 'placeholder/');
        if($handle) {
            while(($path = readdir($handle)) !== false) {
                if($path == '.' || $path == '..') {
                    continue;
                }
                if(preg_match('/^' . $prefix . '_.+\.png$/i', $path)) {
                    unlink(DIR_IMAGE . 'placeholder/' . $path);
                }
            }
        }
        closedir($handle);
    ...
    protected function permission() {
        return $this->user->hasPermission('modify', 'setting/setting');
    }
}
