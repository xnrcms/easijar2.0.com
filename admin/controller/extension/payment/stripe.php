...
class ControllerExtensionPaymentStripe extends Controller
{
    ...
    public function index()
    {
        ...
        if ($this->initStripe() == true) {
            try {
                $currencies = [];
                $supportedCurrencies = \Stripe\CountrySpec::retrieve("US")['supported_payment_currencies'];
    ...
    protected function validate()
    {
        ...
        if (array_get($this->request->post, 'payment_stripe_environment') == 'live') {
            $stripe_secret_key = array_get($this->request->post, 'payment_stripe_live_secret_key');
        } else {
            $stripe_secret_key = array_get($this->request->post, 'payment_stripe_test_secret_key');
        }
