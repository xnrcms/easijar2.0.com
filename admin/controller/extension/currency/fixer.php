...
class ControllerExtensionCurrencyFixer extends Controller {
	...
	public function index() {
		...
		if (isset($this->request->post['currency_fixer_api_key'])) {
			$data['currency_fixer_api_key'] = $this->request->post['currency_fixer_api_key'];
		} else {
			$data['currency_fixer_api_key'] = $this->config->get('currency_fixer_api_key');
		}
	...
	public function currency($default = '') {
		if ($this->config->get('currency_fixer_status')) {
			...
			if ($currencies) {
				...
				curl_setopt($curl, CURLOPT_URL, 'http://data.fixer.io/api/latest?access_key=' . $this->config->get('currency_fixer_api_key'));
				...
				if (isset($response_info['rates'])) {
				    if (isset($response_info['rates'][$default])) {
                        $base_rate = $response_info['rates'][$default];

                        foreach ($currencies as $currency) {
                            if (isset($response_info['rates'][$currency['code']])) {
                                $rate = round($response_info['rates'][$currency['code']] / $base_rate, 8);
                                $this->model_localisation_currency->editValueByCode($currency['code'], $rate);
                            }
                        }
                    }
				}
